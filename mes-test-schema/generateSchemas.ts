...
if (cliArgument === 'generate') {
    ...
    for (let category of categories) {
        ...
        // DO NOT REGENERATE THIS FILE. IT HAS BEEN MODIFIED BY HAND
...
function copyFile(source: string, destination: string) {
    fs.copyFile(source, destination, (err: NodeJS.ErrnoException | null) => {
        if (err) throw err;
    });
}
...
function generateTypescriptInterfaces(schemaLocation: string, saveToLocation: string) {
    ...
    }).then((ts: string) => fs.writeFileSync(saveToLocation, ts))
}
...
function deReferenceJsonSchema(sourcePath: string, targetPath: string): void {
    ...
    fs.writeFile(targetPath, deReferencedSchema, 'utf8', (err: NodeJS.ErrnoException | null) => {
        if (err) {
